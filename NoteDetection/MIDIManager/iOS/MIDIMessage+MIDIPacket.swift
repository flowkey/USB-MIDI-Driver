...
extension MIDIMessage {
    init?(from packet: MIDIPacket) {
        ...
        let (command, key, velocity) = isHighResVelocityMessage ? (command: data.3, key: data.4, velocity: data.5) // ignores high res data
            : (command: data.0, key: data.1, velocity: data.2) // assume normal noteOn/Off

        guard let message = MIDIMessage.from(status: command, data1: key, data2: velocity) else { return nil }
        self = message
    }
}
